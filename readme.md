...
Saat order disimpan, stok produk akan otomatis berkurang sesuai jumlah pembelian.
...
7. Sistem melakukan validasi stok.
8. Sistem membuat data order.
9. Sistem membuat data order item.
10. Sistem mengurangi stok produk.
11. Sistem menampilkan pesan transaksi berhasil.
...
Response berisi data produk, kategori, ukuran, harga, stok, dan gambar.
...
5. Stok produk harus mencukupi.
6. Ukuran produk harus aktif dan sesuai dengan produk.

Jika stok tidak cukup, sistem akan menolak transaksi dan menampilkan pesan error.

---

## Logic Pengurangan Stok

Setiap transaksi berhasil, sistem akan otomatis mengurangi stok produk.

Contoh:

```text
Stok awal Kopi Gula Aren = 10
User membeli 2
Stok akhir = 8
```

Pengurangan stok dilakukan di dalam database transaction agar data tetap aman.
...
Fitur Simple Moving Average tidak digunakan pada versi project ini karena sudah mengalami revisi. Project ini difokuskan menjadi sistem POS berbasis website untuk mendukung transaksi, pengelolaan produk, pengelolaan stok, dan laporan penjualan UMKM Ngunjuk.
