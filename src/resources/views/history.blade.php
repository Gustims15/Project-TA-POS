...
<body>
  <main class="app-shell single-content">
    <aside class="sidebar">
      ...
      <nav class="nav-menu" aria-label="Menu utama">
        ...
        <a class="nav-item active" href="{{ route('frontend.history') }}">
          <span class="nav-icon">◴</span>
          <span>History</span>
        </a>

        <a class="nav-item" href="{{ route('frontend.settings') }}">
          <span class="nav-icon">⚙</span>
          <span>Settings</span>
        </a>
      </nav>
  ...
  <script>
    window.NGUNJUK_ROUTES = {
      ...
      settings: "{{ route('frontend.settings') }}",
      historyApi: "{{ route('api.history.list') }}"
    };

    window.NGUNJUK_CSRF_TOKEN = "{{ csrf_token() }}";
  </script>

  <script src="{{ asset('js/historyy.js') }}"></script>
</body>
</html>